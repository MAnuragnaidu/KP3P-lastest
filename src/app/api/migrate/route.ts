...
import { createPatient, findPatientById } from '@/lib/patient-repository';
  ...
  try {
    ...
    for (const entry of patients) {
      ...
      await createPatient({
        ...
        hbiScoring: strField(patient, 'hbiScoring') || '{}',
        partialMayoScoring: strField(patient, 'partialMayoScoring') || '{}',
        ...
        radiologyInvestigations: strField(patient, 'radiologyInvestigations') || '{}',
        surgeonSurgicalHistory: strField(patient, 'surgeonSurgicalHistory'),
        radiologistFindings: strField(patient, 'radiologistFindings'),
        dietitianDietaryFindings: strField(patient, 'dietitianDietaryFindings'),
        ...
        infectionScreening: strField(patient, 'infectionScreening') || '{"sets":[]}',
        ...
        assessmentComplete: patient.assessmentComplete === true,
        assessmentCurrentStep: intField(patient, 'assessmentCurrentStep', 1),
        userId: intField(patient, 'userId', 0) || null,
        createdAt,
      });
      migrated++;
    }
