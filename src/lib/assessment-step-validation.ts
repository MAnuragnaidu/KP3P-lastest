...
function parseVaccineStatus(raw: unknown): string {
  ...
  if (typeof raw === 'string') {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        return String(parsed.status ?? '').trim();
      }
    } catch {
      return trimmed;
    }
...
function hasRadiologyContent(data: AssessmentFormState): boolean {
  const radiology = parseRadiologyInvestigations(data.radiologyInvestigations);
  return radiology.sets.some((set) => {
    if (set.assessmentDate.trim()) return true;
    return filledRadiologySetEntries(set).length > 0;
  });
}

function hasVaccinationContent(data: AssessmentFormState): boolean {
  return VACCINE_FIELD_KEYS.every((key) => {
    const status = parseVaccineStatus(assessmentField(data, key)).toLowerCase();
    return status === 'given' || status === 'never' || status === 'unknown';
  });
}
...
export function isAssessmentStepComplete(stepNum: number, data: AssessmentFormState): boolean {
  if (stepNum === 5) return hasRadiologyContent(data);
  if (stepNum === 10) return hasVaccinationContent(data);
  return validateAssessmentStepFields(stepNum, data).errors.length === 0;
}
...
export function validateAssessmentStepFields(
  ...
  if (stepNum === 7) {
    pushMissing(
      errors,
      'surgeonSurgicalHistory',
      'Surgical History',
      !isNonEmpty(assessmentField(data, 'surgeonSurgicalHistory')),
    );
  }
  ...
  if (stepNum === 8) {
    pushMissing(
      ...
      'radiologistFindings',
      'Radiology Findings',
      !isNonEmpty(assessmentField(data, 'radiologistFindings')),
    );
  }
  ...
  if (stepNum === 9) {
    pushMissing(
      errors,
      'dietitianDietaryFindings',
      'Dietary Findings',
      !isNonEmpty(assessmentField(data, 'dietitianDietaryFindings')),
    );
  }
