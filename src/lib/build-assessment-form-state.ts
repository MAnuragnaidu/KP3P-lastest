...
export function buildAssessmentFormState(patient: PatientWithUser): AssessmentFormState {
  ...
  try {
    if (typeof previousSurgeries === 'string') {
      ...
      previousSurgeries = Array.isArray(p)
        ? normalizePreviousSurgeryLabels(sanitizeNoneExclusiveSelection(p as string[]))
        : previousSurgeries;
    } else if (Array.isArray(previousSurgeries)) {
      previousSurgeries = normalizePreviousSurgeryLabels(
        sanitizeNoneExclusiveSelection(previousSurgeries),
      );
    }
  } catch {
    /* keep string */
...
export function buildAssessmentSavePayload(
  ...
  for (const key of ['previousSurgeries', 'comorbidities', 'extraintestinalManif'] as const) {
    if (key in payload && Array.isArray(payload[key])) {
      const sanitized = sanitizeNoneExclusiveSelection(payload[key] as string[]);
      payload[key] =
        key === 'previousSurgeries' ? normalizePreviousSurgeryLabels(sanitized) : sanitized;
    }
  }
