...
function normalizePreviousSurgeriesJson(val: unknown): string {
  let values: string[] = [];
  if (Array.isArray(val)) {
    values = val.filter((item): item is string => typeof item === 'string');
  ...
  return JSON.stringify(
    normalizePreviousSurgeryLabels(sanitizeNoneExclusiveSelection(values)),
  );
}
...
export function patientCreateDataFromBody(body: Record<string, unknown>): PatientCreateInput {
  ...
  return {
    ...
    previousSurgeries: normalizePreviousSurgeriesJson(b.previousSurgeries),
    ...
    surgeonSurgicalHistory:
      typeof b.surgeonSurgicalHistory === 'string' ? b.surgeonSurgicalHistory.trim() : '',
    radiologistFindings:
      typeof b.radiologistFindings === 'string' ? b.radiologistFindings.trim() : '',
    dietitianDietaryFindings:
      typeof b.dietitianDietaryFindings === 'string' ? b.dietitianDietaryFindings.trim() : '',
