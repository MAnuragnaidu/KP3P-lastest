...
export const DUPLICATE_PATIENT_EMAIL_MESSAGE =
  'This email address is already registered. Contact your gastroenterologist if you need to update your information.';

export const DUPLICATE_PATIENT_PHONE_MESSAGE =
  'This phone number is already registered. Contact your gastroenterologist if you need to update your information.';

export type IntakeDuplicateField = 'email' | 'contactPhone';
...
/** Case-insensitive email + normalized phone match on the same patient record. */
...
/** Intake duplicate check: reject if email or phone already exists on any patient. */
export async function findIntakeDuplicatePatient(
  email: string,
  contactPhone: string,
): Promise<IntakeDuplicateField | null> {
  const supabase = getSupabaseAdminClient();
  const trimmedEmail = email.trim();
  const normalizedPhone = normalizeContactPhoneForMatch(contactPhone);
  ...
  const [{ data: emailMatches, error: emailError }, { data: phoneRows, error: phoneError }] =
    await Promise.all([
      supabase.from('Patient').select('id').ilike('email', trimmedEmail).limit(1),
      supabase
        .from('Patient')
        .select('id,contactPhone')
        .or(`contactPhone.eq.${normalizedPhone},contactPhone.ilike.%${normalizedPhone}`),
    ]);

  if (emailError) {
    throw new Error(`Supabase Patient email duplicate lookup failed: ${emailError.message}`);
  }
  if (phoneError) {
    throw new Error(`Supabase Patient phone duplicate lookup failed: ${phoneError.message}`);
  }

  const emailExists = (emailMatches ?? []).length > 0;
  const phoneExists = (phoneRows ?? []).some(
    (row) =>
      typeof row.contactPhone === 'string' &&
      normalizeContactPhoneForMatch(row.contactPhone) === normalizedPhone,
  );
  ...
  if (phoneExists) return 'contactPhone';
  return null;
}

export function duplicateMessageForIntakeField(field: IntakeDuplicateField): string {
  return field === 'email' ? DUPLICATE_PATIENT_EMAIL_MESSAGE : DUPLICATE_PATIENT_PHONE_MESSAGE;
}
